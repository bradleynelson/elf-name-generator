...
export class DwarvenNameGenerator {
    ...
    _generateFirstName(gender, subrace = "general") {
        ...
        if (gender !== "neutral") {
            prefixCandidates = this.firstNamePrefixes.filter((c) => c.gender === gender || c.gender === "neutral");
            suffixCandidates = this.firstNameSuffixes.filter((c) => c.gender === gender || c.gender === "neutral");
        }
    ...
    _smoothConsonantCluster(prefix, suffix) {
        ...
        if (
            !vowels.has(lastChar) &&
            lastChar === lastTwoChars.charAt(0) &&
            !vowels.has(firstChar) &&
            firstChar !== lastChar &&
            lastChar !== "n"
        ) {
