...
export class UIController {
    ...
    displayName(nameData, generatorType = "elven") {
        ...
        if (generatorType === "dwarven" && nameData.firstName && nameData.clanName) {
            ...
            this.elements.nameMeaning.innerHTML =
                firstNameMeaning + '<span class="meaning-separator"> </span>' + clanNameMeaning;
        } else {
        ...
        if (canPronounce) {
            ...
            if (this.elements.namePronunciation) {
                this.elements.namePronunciation.textContent =
                    generatorType === "halfling" || generatorType === "orc"
    ...
    _buildStackedMeaning(meaning) {
        ...
        const stackedParts = parts.map((part) => {
            ...
            const optionsHTML = alternatives.map((alt) => `<span class="meaning-option">${alt}</span>`).join("");
    ...
    _displayBreakdown(nameData) {
        ...
        if (nameData.components && nameData.components.length > 0) {
            ...
            for (let i = 0; i < nameData.components.length; i++) {
                ...
                if (
                    i < nameData.components.length - 1 &&
                    nameData.connectors &&
    ...
    _displayModifierSuggestions(nameData, showGenderPrefix, showFinalVowel) {
        ...
        if (showGenderPrefix) {
            const genderPrefixOptionsHTML = GENDER_PREFIX_VOWELS.map(
                (v) => `
        ...
        if (showFinalVowel) {
            const vowelOptionsHTML = FINAL_VOWELS.map(
                (v) => `
                <div class="vowel-option" data-vowel="${v.vowel}">
    ...
    applyGenderPrefix(vowel) {
        ...
        html = html.replace(/(<span class="component-label">Syllables:<\/span>\s+)\d+/, `$1${newSyllables}`);
    ...
    displayFavorites(favorites) {
        ...
        this.elements.favoritesList.innerHTML = filteredFavorites
            .map((fav, _index) => {
    ...
    _speakName() {
        ...
        try {
            ...
            const preferredVoices = voices.filter((voice) => voice.lang.startsWith("en") && voice.localService);
